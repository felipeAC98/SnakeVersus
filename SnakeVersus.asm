...
DESENHACARACTERE PROC USES ecx
	 ...
	 mul edx                               ;// Multiplica edx com eax e coloca o resultado em eax.Ele multiplica pois para chegar na linha Y, eh necessario andar
										   ;// no vetor buffer a seguinte quantidade: posicao Y*tamanho da linha(numero de colunas)
	 
	 add eax, [esi + 4]                    ;// Somando em eax (que neste momento possui o valor multiplicado de Y com a quantidade de elmentos por linha) o valor 
										   ;// da posicao X
...
transformaImpressao PROC
	 ...
	 mov eax, ultimaPosBuffer;      ;// Endereco no qual devera estar apontando a proxima posicao que algo sera impresso no buffer, alguns elementos no jogo sao 
									;// impressos posteriormente e assim nao ficam no topo da tela, como por exemplo no menu inicial quando eh escrito SNAKE e depois 
									;// VERSUS
	 mov edx, 0
...
alteraCores PROC
	 ...
	 and dx, 00001111b       ;// O atributo da cor do caracter possui 1byte mas a parte alta e a parte baixa controlam o fundo e a cor do caractere reespectivamente,
							 ;// nos somente vamos alterar a cor do caractere, devido a isso fazemos um and para deixar preto o fundo
